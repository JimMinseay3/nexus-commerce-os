...
export default function DashboardPage() {
  ...
  return <>
    ...
    <div className="section-grid">
      <div className="section-card"><div className="section-title"><span>销售趋势</span><Tag color="purple">多平台汇总</Tag></div><div className="trend-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data.trend}><defs><linearGradient id="sales" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#6758e8" stopOpacity={.3}/><stop offset="95%" stopColor="#6758e8" stopOpacity={0}/></linearGradient></defs><CartesianGrid vertical={false} stroke="#edf0f5"/><XAxis dataKey="day" tick={{fontSize:11}} axisLine={false}/><YAxis tick={{fontSize:11}} axisLine={false}/><Tooltip formatter={(v) => money(String(v))}/><Area type="monotone" dataKey="sales" stroke="#6758e8" strokeWidth={2.5} fill="url(#sales)"/></AreaChart></ResponsiveContainer></div></div>
      <div className="section-card"><div className="section-title"><span>需要关注</span><span style={{fontSize:12,color:'#8a9690'}}>实时</span></div>
        ...
        <div className="alert-row"><div className="alert-dot" style={{background:'#6758e8'}}/><div><strong>{data.pending_purchase_approvals} 张采购单待审批</strong><span>审批后才能进入采购执行</span></div></div>
        <div className="alert-row"><div className="alert-dot" style={{background:data.failed_sync_jobs ? '#d04d3c':'#22b8a8'}}/><div><strong>{data.failed_sync_jobs} 个同步任务失败</strong><span>{data.failed_sync_jobs ? '请查看连接器日志并重试':'所有平台同步状态正常'}</span></div></div>
      </div>
    </div>
  </>
}
